...
// Must stay directly below reflect-metadata and above everything else. Importing
// this module starts OpenTelemetry immediately, before AppModule can load ioredis.
import { tracingStarted } from '@infrastructure/observability/tracing.preload';
...
import { telemetryStatusLine } from '@infrastructure/observability/telemetry-status';
...
async function bootstrap(): Promise<void> {
  ...
  // Whether each signal is leaving the process is not something anyone should
  // have to infer from an empty dashboard — and the three pipelines are
  // independent, so one being on says nothing about the others.
  new Logger('Bootstrap').log(telemetryStatusLine(config.observability, tracingStarted));
