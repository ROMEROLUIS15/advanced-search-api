...
- [ ] 1.1 Against the local stack (`docker compose up -d elasticsearch redis`), confirm `mappings._meta`
...
- [ ] 1.2 Confirm the same three operations on **Elastic Cloud Serverless**, which is what production runs
...
- [ ] 1.3 Confirm the stale docstring in `product-index.mapping.ts`: it claims "1 shard / 0 replicas" while
...
- [ ] 2.1 Add `index-definition.fingerprint.ts` beside the adapter: canonical serialization (keys sorted)
...
- [ ] 2.2 Unit spec: identical definitions hash equal; a changed analyzer, a changed field type and a
...
- [ ] 2.3 Add `physical-index.version.ts`: parse `<alias>_v<n>` from a physical index name, resolve the next
      version from the live one, and return 1 when there is no alias. An unparseable name is an error, not
      a guess (D44).
- [ ] 2.4 Unit spec for the parser and the resolver, including the no-alias case and the unparseable case.

## 3. Port and application models

- [ ] 3.1 Add the `IndexPreparation` / `IndexPublication` models under `application/models/` — action,
      version and pruned versions only. No index names, no Elasticsearch types.
- [ ] 3.2 Change `ProductIndexPort`: `ensureIndex()` returns `IndexPreparation`, and add
      `publishIndex(): Promise<IndexPublication>`. Update the docstrings to say what each guarantees.

## 4. Adapter: the migration path

- [ ] 4.1 Replace the `PHYSICAL_INDEX_SUFFIX` constant and the early return in `ensureIndex()` with the
...
- [ ] 4.2 Make `bulkIndex` and `refresh` address the pending target when a migration is in flight and the
      alias otherwise. This is the step that makes a migration load the new index instead of the one being
      replaced.
- [ ] 4.3 Implement `publishIndex()`: one `indices.updateAliases` carrying remove+add (D45), then prune
      versions older than the one just replaced (D47). A no-op when nothing is pending.
- [ ] 4.4 Handle the concurrency case: `resource_already_exists_exception` on the new version means another
      migration is in flight — fail with a clear typed error instead of loading into it (design risk list).
- [ ] 4.5 Keep the adapter under `max-lines: 250`; if it does not fit, the fingerprint/version helpers take
      the overflow, not a raised cap.
- [ ] 4.6 Extend `product-index.adapter.spec.ts` over the mocked client: unchanged fingerprint issues no
...
- [ ] 5.1 `SeedCatalogUseCase`: prepare, load, refresh, then publish **only when zero documents failed**
      (D46). On any failure the alias stays where it is and the existing non-zero exit is kept.
- [ ] 5.2 Update `seed-catalog.use-case.spec.ts` for both paths — published on a clean load, not published
      when a document fails.
- [ ] 5.3 Make the seed's log line state what happened: unchanged, created, or migrated from version N to
      N+1. This is the only place an operator sees that a migration occurred.

## 6. Integration coverage against a real Elasticsearch

- [ ] 6.1 New integration spec: seed a definition, change it, seed again, and assert a new physical version
      exists, the alias points at it, and the previous one is retained.
- [ ] 6.2 Assert the alias resolves to exactly one index throughout — poll it during the migration, since
      an atomic flip is precisely what a single-call `updateAliases` buys and a two-call version would pass
      every other assertion here.
- [ ] 6.3 Assert a product removed from the dataset is gone after the migration, and that one still present
      is served (the D48 rebuild, proved rather than argued).
- [ ] 6.4 Assert an incomplete load leaves the alias untouched (D46).
- [ ] 6.5 Confirm D49 empirically: cache scopes are unchanged by a migration, so a cached search can serve
...
- [ ] 7.1 `CLAUDE.md`: the D1 index bullet describes a versioned index behind an alias — extend it with the
...
- [ ] 7.2 `README.md`: the seed section gains what a migration does and the rollback runbook from D47 (the
      literal `_aliases` call), plus the note that a migration is still a manual pre-boot step.
- [ ] 7.3 `docs/PENDING-2026-07-28.md`: close B3, stating what shipped and what did not — the seed is still
      manual and cache invalidation is still TTL-bound by choice.
- [ ] 7.4 Run the local gate — `npm run lint:ci && npm run test:cov && npm run build` — plus
