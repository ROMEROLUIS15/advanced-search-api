...
Dependency-aware health reporting for container probes, split by audience: a full report for humans and
monitors, a critical-only readiness endpoint for the platform that polls continuously, and a dependency-free
liveness endpoint. Elasticsearch is the critical dependency, Redis the non-critical one.
## Requirements
