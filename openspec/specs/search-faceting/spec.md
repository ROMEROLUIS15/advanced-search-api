# search-faceting Specification

## Purpose

Faceted navigation returned alongside every `/search` response: category, subcategory, location and
price-range bucket counts, computed so that a dimension the user has already narrowed can still be
widened (a facet is not constrained by its own filter).
